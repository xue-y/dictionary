...
return array(
   // "table"     =>          array(),
    "ext"       =>          "html|csv",     //导出文件格式
    "csvIsTit" =>          false,           //csv格式文件是否输入title,默认不输出
    "limit"     =>          300,              // 如果分卷每卷的表个数 限制
    "minLimit" =>           50,              // 如果分卷最少表个数限制
    "maxLimit" =>           500,            // 最大表个数
    ...
    'fieldTitKey' =>        '字段,类型,是否为NULL,Key,默认,额外,注释',  // 数据表字段tit 要与 val 中的取值一一对应 Privileges
    'fieldTitVal' =>        'Field,Type,Null,Key,Default,Extra,Comment'   // 如果需要添加显示字段对应添加即可  权限
);